...
menambah immutable audit log dan M3-01 menambah foundation versi SPMI; workflow
target lainnya masih bertahap. Dokumen ini merupakan hasil penelusuran source
...
| A-03 | Dokumen SPMI, standar, pertanyaan, instrumen, penetapan | Confidential / Internal | M3-01 menyediakan foundation versi + private source; master standar/pertanyaan legacy masih mutable dan sebagian file mempunyai fallback legacy. |
...
| A-01, A-03, A-08, A-09 | Admin/operator atau cascade database | **TM-25 Hard delete/cascade menghilangkan histori** melalui delete user, standar, pertanyaan, periode, atau tugas. | Critical — audit lama tidak dapat direkonstruksi dan actor/provenance hilang. | M3-01 memberi `spmi_versions` empat FK RESTRICT dan trigger no-delete; active content juga immutable. Master legacy tetap memakai cascade/hard-delete dan belum di-backfill. | M3-02 wajib mempertahankan history guard serta audit transition; milestone standard/snapshot berikutnya harus melakukan expand/backfill/verify sebelum cutover legacy. | Smoke M3-01 menolak edit active dan hard delete serta membuktikan retirement/replacement. Tetap uji cascade legacy dan reconciliation pada clone sebelum cutover. |
