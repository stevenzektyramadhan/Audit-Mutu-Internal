...
| A-04, A-05, A-08, A-11 | Pengguna login yang menebak ID | **TM-05 IDOR** pada URL tugas, jawaban, bukti, laporan, RTM, dan tindak lanjut. | High — baca/ubah objek milik unit atau assignment lain. | M1-04 memusatkan policy assignment/evidence dan scoped owner query. M2-02 menambahkan direct dated membership API, membatasi target pengelolaan assignment, dan menguji denial Auditor. Modul admin lama tetap institution-wide sampai M2-03; RTM belum ada. | M2-03 harus memetakan capability ke direct/hierarchical scope secara eksplisit, lalu menerapkannya konsisten pada list, mutation, download, export, RTM, dan tindak lanjut. | Gunakan dua user, dua unit, dua assignment, ID berurutan dan ID tidak ada; uji GET/POST/AJAX/download/export. Respons ditolak seragam dan tidak membocorkan metadata. |
...
| A-03 sampai A-11 | Pengguna login dan endpoint yang hanya role-check | **TM-21 Broken object-level authorization** pada list, count, search, bulk action, export, file, dan objek turunan. | High/Critical — kebocoran atau mutasi lintas organisasi dalam skala besar. | M1-04 menyediakan central capability/object policy, scoped participant list/count/detail/mutation/file, deny-default, dan explicit logged override. M2-02 menyediakan membership langsung aktif; admin lama tetap global sampai matriks M2-03 mengikat capability ke scope. RTM/PIC belum mempunyai surface dan selalu ditolak policy. | Terapkan active membership scope yang sama pada seluruh list, count, search, bulk, export, file, serta objek RTM/tindak lanjut; definisikan parent/descendant dan separation of duties sebelum membuka akses. | Matriks role × capability × unit × ownership × state untuk list/detail/mutation/bulk/export/download; hitungan dan pesan error tidak boleh membocorkan objek di luar scope. |
