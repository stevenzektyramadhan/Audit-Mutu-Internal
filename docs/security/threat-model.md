...
Threat model ini mengidentifikasi ancaman utama sepanjang implementasi
versioning SPMI, snapshot audit, RTM, tindak lanjut, dan audit log. M1 sudah
menambah immutable audit log; M3-01/M3-02 menambah versi dan workflow SPMI;
M3-03 menambah master standar milik versi. Workflow target lainnya masih
bertahap. Dokumen ini merupakan hasil penelusuran source
code dan baseline lokal, bukan penetration test atau bukti kesiapan produksi.
...
| A-03 | Dokumen SPMI, standar, pertanyaan, instrumen, penetapan | Confidential / Internal | M3-01–M3-03 menyediakan versi + private source + master standar versioned. Master standar/pertanyaan legacy masih mutable karena cutover belum dilakukan dan sebagian file mempunyai fallback legacy. |
...
| A-01, A-03, A-08, A-09 | Admin/operator atau cascade database | **TM-25 Hard delete/cascade menghilangkan histori** melalui delete user, standar, pertanyaan, periode, atau tugas. | Critical — audit lama tidak dapat direkonstruksi dan actor/provenance hilang. | `spmi_versions` memakai empat FK RESTRICT dan no-delete trigger; active content immutable. M3-03 menambah RESTRICT FK serta draft-only/no-delete trigger untuk `spmi_standards`. Master legacy tetap memakai cascade/hard-delete dan belum di-backfill. | Milestone statement/snapshot berikutnya harus mempertahankan guard dan melakukan expand/backfill/verify sebelum cutover legacy. | Smoke menolak edit standar review/active dan hard delete, membuktikan retirement/replacement, serta memverifikasi clone 21 standar independen. Tetap uji cascade legacy dan reconciliation sebelum cutover. |
