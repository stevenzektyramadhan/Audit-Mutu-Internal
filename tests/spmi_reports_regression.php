...
foreach (['score_recap_per_indicator_per_auditee', 'r.auditee_name_snapshot AS auditee_name', 'r.source_standard_code_snapshot AS standard_code', 'ri.indicator_code_snapshot AS item_code', 'ri.indicator_title_snapshot AS item_title', 'ri.score AS score', "from('spmi_reports r')", "join('spmi_report_items ri', 'ri.report_id = r.id')", "where('r.cycle_code_snapshot', \$cycle_code)", "order_by('r.auditee_name_snapshot', 'ASC')", "order_by('r.source_standard_code_snapshot', 'ASC')", "order_by('ri.display_order', 'ASC')"] as $literal) m10_check(strpos($model, $literal) !== FALSE, 'Radar recap score model contract missing: ' . $literal);
