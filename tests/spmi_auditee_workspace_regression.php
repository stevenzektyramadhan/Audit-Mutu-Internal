...
foreach (['current parity migration 001-030', 'spmi_auditee_submissions', 'spmi_auditee_submission_items', 'spmi_auditee_evidence', 'uq_spmi_auditee_submissions_assignment', 'uq_spmi_auditee_submission_items_item', 'fk_spmi_auditee_evidence_submission_item'] as $literal) m8_check(strpos($schema, $literal) !== FALSE, 'M8 schema parity missing: ' . $literal);
...
foreach (['auditor_evidence_snapshot', 'json_decode', 'html_escape'] as $literal) m8_check(strpos($final_result_view, $literal) !== FALSE, 'M17-07B final-result auditor evidence metadata snapshot missing: ' . $literal);
m8_check(strpos($final_result_view, 'stored_name') === FALSE && strpos($final_result_view, 'auditor-evidence/') === FALSE, 'M17-07B final-result must not expose live auditor evidence access.');
foreach (['improvement_plan_snapshot', 'evidence_date_snapshot', 'Rencana perbaikan', 'Tanggal bukti', 'html_escape'] as $literal) m8_check(strpos($final_result_view, $literal) !== FALSE, 'M17-07C final-result must safely display report snapshot: ' . $literal);
