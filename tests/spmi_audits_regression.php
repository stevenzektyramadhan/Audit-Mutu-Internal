...
spmi_audit_check(substr_count($views[3], 'name="source_package_id"') === 1 && strpos($views[3], 'type="hidden" id="source_package_id" name="source_package_id" required') !== FALSE, 'M7 assignment package picker must retain one required hidden source_package_id field.');
spmi_audit_check(strpos($views[3], '<select id="source_package_id"') === FALSE && strpos($views[3], '<option value="">Pilih paket</option>') === FALSE, 'M7 assignment package picker must not restore the native package select.');
spmi_audit_check(strpos($views[3], 'for="source_package_picker">Paket Instrumen</label>') !== FALSE && strpos($views[3], 'id="source_package_picker"') !== FALSE && strpos($views[3], 'placeholder="Ketik kode atau nama paket..."') !== FALSE, 'M7 assignment package picker must expose the unified combobox.');
spmi_audit_check(strpos($views[3], 'role="combobox"') !== FALSE && strpos($views[3], 'role="listbox"') !== FALSE && strpos($views[3], 'Paket instrumen ditemukan: ') !== FALSE && strpos($views[3], 'Paket tidak ditemukan') !== FALSE && strpos($views[3], 'id="source_package_clear"') !== FALSE, 'M7 assignment package picker must provide count, empty state, and clear control.');
spmi_audit_check(strpos($views[3], '$packages') !== FALSE && strpos($views[3], 'html_escape($package->version_code') !== FALSE && strpos($views[3], 'data-package-id') !== FALSE && strpos($views[3], "packageId.value = option.getAttribute('data-package-id')") !== FALSE, 'M7 assignment package picker must submit only an escaped real package ID.');
spmi_audit_check(strpos($views[3], 'source_package_search') === FALSE && strpos($views[3], 'source_package_search_status') === FALSE && strpos($views[3], 'source_package_results') === FALSE && strpos($views[3], 'source_package_empty') === FALSE, 'M7 assignment package picker must remove legacy search controls.');
spmi_audit_check(strpos($views[3], 'name="auditor_id"') !== FALSE && strpos($views[3], 'name="auditee_id"') !== FALSE && strpos($views[3], 'Buat snapshot penugasan') !== FALSE, 'M7 assignment picker must retain auditor, auditee, and snapshot controls.');
