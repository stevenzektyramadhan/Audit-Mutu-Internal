...
- 2026-09-22 - Sisyphus: Fixed the active SPMI Auditee lower final-submit button by replacing duplicate IDs with one shared selector and binding the existing read-only confirmation navigation to both controls; auditee/audits/hardening regressions and PHP lint passed. Authenticated click-through QA remains blocked without an auditee session.
- 2026-09-22 - Sisyphus: Aligned the SPMI indicator create/edit form with existing Standards form primitives, preserving its POST/CSRF protocol, values, validation, and routes; focused indicator/evidence-policy/schema/hardening/sidebar/auth guards passed. Authenticated responsive browser QA remains blocked by unavailable credentials and shared-browser contention.
