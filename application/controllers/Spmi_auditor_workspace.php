...
class Spmi_auditor_workspace extends CI_Controller
{
    ...
    protected function output_evidence_file($file, $not_found_message) { if ($file['backend'] === 'google_drive') { header('Content-Type: ' . $file['mime']); header('Content-Length: ' . (string) $file['size']); header('Content-Disposition: attachment; filename="' . rawurlencode($file['name']) . '"'); header('Cache-Control: private, no-store, max-age=0'); header('Pragma: no-cache'); header('Expires: 0'); $result = $this->service->stream_drive_download($file['drive_file_id'], fopen('php://output', 'wb')); if (!$result['success']) { show_error($not_found_message, 404, 'Not Found'); return; } return; } header('Content-Type: ' . $file['mime']); header('Content-Length: ' . (string) filesize($file['path'])); header('Content-Disposition: attachment; filename="' . rawurlencode($file['name']) . '"'); header('Cache-Control: private, no-store, max-age=0'); header('Pragma: no-cache'); header('Expires: 0'); readfile($file['path']); }
}
