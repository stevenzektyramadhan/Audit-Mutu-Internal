...
class Spmi_auditor_workspace extends CI_Controller
{
    ...
    public function download_evidence($id) { $file = $this->service->download((int) $id, $this->user_id()); if (!$file) { show_error('Bukti tidak ditemukan.', 404, 'Not Found'); return; } $this->output_evidence_file($file, 'Bukti tidak ditemukan.'); }
    ...
    public function download_auditor_evidence($id) { $file = $this->service->download_auditor_evidence((int) $id, $this->user_id()); if (!$file) { show_error('Bukti auditor tidak ditemukan.', 404, 'Not Found'); return; } $this->output_evidence_file($file, 'Bukti auditor tidak ditemukan.'); }
    ...
    protected function output_evidence_file($file, $not_found_message) { if ($file['backend'] === 'google_drive') { $temp = fopen('php://temp', 'w+b'); $result = $this->service->stream_drive_download($file['drive_file_id'], $temp); if (!$result['success']) { fclose($temp); show_error($not_found_message, 404, 'Not Found'); return; } rewind($temp); $stat = fstat($temp); header('Content-Type: ' . $file['mime']); header('Content-Length: ' . (string) $stat['size']); header('Content-Disposition: attachment; filename="' . rawurlencode($file['name']) . '"'); header('Cache-Control: private, no-store, max-age=0'); header('Pragma: no-cache'); header('Expires: 0'); $output = fopen('php://output', 'wb'); stream_copy_to_stream($temp, $output); fclose($output); fclose($temp); return; } header('Content-Type: ' . $file['mime']); header('Content-Length: ' . (string) filesize($file['path'])); header('Content-Disposition: attachment; filename="' . rawurlencode($file['name']) . '"'); header('Cache-Control: private, no-store, max-age=0'); header('Pragma: no-cache'); header('Expires: 0'); readfile($file['path']); }
}
