...
class Spmi_auditor_workspace_model extends CI_Model
{
    ...
    {
        return $this->db->select('a.id, a.source_package_code, a.source_package_title, c.cycle_code, c.title AS cycle_title, c.state, s.status AS submission_status, aa.status AS assessment_status')
            ->from('spmi_audit_assignments a')->join('spmi_audit_cycles c', 'c.id = a.cycle_id')
            ->join('spmi_auditee_submissions s', 's.assignment_id = a.id')
            ->join('spmi_auditor_assessments aa', 'aa.assignment_id = a.id', 'left')
            ->where('a.auditor_id', (int) $user_id)->where('s.status', 'submitted')
            ->where_in('c.state', ['configured', 'closed'])->order_by('c.start_date', 'DESC')->order_by('a.id', 'ASC')->get()->result();
    }

    public function assignment($assignment_id, $user_id, $for_update = FALSE)
    {
        $sql = 'SELECT a.*, c.cycle_code, c.title AS cycle_title, c.state, s.id AS submission_id, s.status AS submission_status, s.version AS submission_version, aa.id AS assessment_id, aa.status AS assessment_status, aa.version AS assessment_version FROM spmi_audit_assignments a JOIN spmi_audit_cycles c ON c.id = a.cycle_id JOIN spmi_auditee_submissions s ON s.assignment_id = a.id LEFT JOIN spmi_auditor_assessments aa ON aa.assignment_id = a.id WHERE a.id = ? AND a.auditor_id = ? AND s.status = ? AND c.state IN (?, ?)';
        if ($for_update) $sql .= ' FOR UPDATE';
        return $this->db->query($sql, [(int) $assignment_id, (int) $user_id, 'submitted', 'configured', 'closed'])->row();
    }
    ...
    public function submission_for_update($assignment_id) { return $this->db->query('SELECT * FROM spmi_auditee_submissions WHERE assignment_id = ? AND status = ? FOR UPDATE', [(int) $assignment_id, 'submitted'])->row(); }
    public function assessment_for_update($assignment_id, $user_id) { return $this->db->query('SELECT aa.* FROM spmi_auditor_assessments aa JOIN spmi_audit_assignments a ON a.id = aa.assignment_id WHERE aa.assignment_id = ? AND a.auditor_id = ? FOR UPDATE', [(int) $assignment_id, (int) $user_id])->row(); }
    ...
    public function submitted_items($assignment_id, $user_id) { return $this->db->query('SELECT si.assignment_item_id, si.realization FROM spmi_auditee_submission_items si JOIN spmi_auditee_submissions s ON s.id = si.submission_id JOIN spmi_audit_assignments a ON a.id = s.assignment_id WHERE s.assignment_id = ? AND a.auditor_id = ? AND s.status = ? ORDER BY si.assignment_item_id ASC', [(int) $assignment_id, (int) $user_id, 'submitted'])->result(); }
    public function assignment_items($assignment_id, $user_id) { return $this->db->select('i.*')->from('spmi_audit_assignment_items i')->join('spmi_audit_assignments a', 'a.id = i.assignment_id')->where('i.assignment_id', (int) $assignment_id)->where('a.auditor_id', (int) $user_id)->order_by('i.display_order', 'ASC')->get()->result(); }
    ...
    public function evidence($assignment_item_id, $user_id) { return $this->db->select('e.*')->from('spmi_auditee_evidence e')->join('spmi_auditee_submission_items si', 'si.id = e.submission_item_id')->join('spmi_auditee_submissions s', 's.id = si.submission_id')->join('spmi_audit_assignments a', 'a.id = s.assignment_id')->where('si.assignment_item_id', (int) $assignment_item_id)->where('a.auditor_id', (int) $user_id)->where('s.status', 'submitted')->get()->result(); }
    public function assessment($assignment_id, $user_id) { return $this->db->select('aa.*')->from('spmi_auditor_assessments aa')->join('spmi_audit_assignments a', 'a.id = aa.assignment_id')->where('aa.assignment_id', (int) $assignment_id)->where('a.auditor_id', (int) $user_id)->get()->row(); }
    ...
    public function create_assessment($assignment_id) { return $this->db->insert('spmi_auditor_assessments', ['assignment_id' => (int) $assignment_id]) ? (int) $this->db->insert_id() : 0; }
