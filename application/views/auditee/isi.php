...
<?php echo form_open('auditee/simpan_jawaban/' . (int) $tugas->id); ?>
    <?php foreach ($jawaban as $index => $item): ?>
        ...
        $safe_link = ami_safe_http_url($item->link_bukti);
        ?>
        <div class="ami-panel mb-3">
            <div class="ami-panel-body">
                ...
                <div class="form-group">
                    ...
                    <textarea id="jawaban-<?php echo (int) $item->id; ?>" name="jawaban[<?php echo (int) $item->id; ?>]" rows="3" class="form-control bg-dark text-light border-secondary" <?php echo $read_only ? 'readonly' : ''; ?>><?php echo ami_e($current_jawaban); ?></textarea>
                </div>
                <div class="form-group<?php echo $sudah_dinilai ? '' : ' mb-0'; ?>">
                    ...
                    <input id="link-<?php echo (int) $item->id; ?>" type="url" name="link_bukti[<?php echo (int) $item->id; ?>]" value="<?php echo ami_e($current_link); ?>" class="form-control bg-dark text-light border-secondary" placeholder="https://..." required <?php echo $read_only ? 'readonly' : ''; ?>>
                    <?php if ($read_only && $safe_link !== ''): ?>
                        <a class="d-inline-block mt-2" href="<?php echo ami_e($safe_link); ?>" target="_blank" rel="noopener noreferrer">
                ...
                <?php if ($sudah_dinilai): ?>
                    <div class="row pt-3" style="border-top:1px solid var(--ami-border);">
                        <div class="col-md-3 mb-2 mb-md-0">
                            ...
                            <div class="ami-stat-value" style="font-size:20px;"><?php echo ami_e((string) $item->skor); ?> / 4</div>
                        </div>
                        <div class="col-md-9">
                            <div class="ami-stat-label">Catatan atau temuan</div>
                            <div><?php echo nl2br(ami_e($item->temuan ?: 'Tidak ada catatan.')); ?></div>
                        </div>
                    </div>
                <?php endif; ?>
            </div>
