...
class Spmi_auditee_workspace_service
{
    ...
    protected function validate_file($file) { $limit_bytes = $this->upload_limit_bytes('spmi_evidence'); $limit_mib = $this->limit_mib($limit_bytes); $message = 'Bukti harus berupa PDF, JPEG, atau PNG maksimal ' . $limit_mib . ' MiB.'; if (!is_array($file) || !isset($file['error'], $file['tmp_name'], $file['size'], $file['name']) || $file['error'] !== UPLOAD_ERR_OK || !is_uploaded_file($file['tmp_name']) || (int) $file['size'] > $limit_bytes) return ['success' => FALSE, 'message' => $message]; $finfo = finfo_open(FILEINFO_MIME_TYPE); $mime = $finfo ? finfo_file($finfo, $file['tmp_name']) : FALSE; if ($finfo) finfo_close($finfo); $image = @getimagesize($file['tmp_name']); $allowed = ['application/pdf' => 'pdf', 'image/jpeg' => 'jpg', 'image/png' => 'png']; if (!isset($allowed[$mime]) || (($mime === 'image/jpeg' || $mime === 'image/png') && (!$image || $image['mime'] !== $mime))) return ['success' => FALSE, 'message' => $message]; return ['success' => TRUE, 'mime' => $mime, 'extension' => $allowed[$mime]]; }
    ...
    public function delete($evidence_id, $user_id, $version)
    {
        ...
        if ($result['success'] && $this->is_drive_evidence($evidence)) { $trash = $this->drive_storage()->trash($evidence->drive_file_id); if (!$trash['success']) $this->record_drive_trash_failure('spmi_auditee_evidence', (int) $evidence->id, $evidence->drive_file_id, $evidence->drive_folder_id, $evidence->stored_name); }
    ...
    public function upload_limit_mib() { return $this->limit_mib($this->upload_limit_bytes('spmi_evidence')); }
    protected function upload_limit_bytes($category) { require_once APPPATH . 'services/Upload_size_settings_service.php'; return (int) Upload_size_settings_service::limit_bytes($category); }
